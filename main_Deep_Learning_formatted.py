    ...
    for i in range(feature_values.shape[1]):
        if i != 6 and i != 1:  # Skip the transformed features
            normalized_values[:, i] = np.where(
                earth_values[i] != 0, feature_values[:, i] / (earth_values[i] + epsilon), 0
            )
...
balanced_df = balanced_df[['pl_name'] + feature_columns]

# Remove rows where all defined feature columns have missing values
balanced_df = balanced_df.dropna(subset=feature_columns, how='all')
...
DL_Model.add(Dense(128, input_shape=(X_train.shape[1],), kernel_regularizer=l1_l2(l1=0.005, l2=0.01)))
...
DL_Model.add(Dense(1, activation='sigmoid', kernel_regularizer=l1_l2(l1=0.005, l2=0.01)))
